\
#pragma once
#include <bits/stdc++.h>
#include <mysql/mysql.h>
#include <openssl/sha.h>
using namespace std;

// --- CGI ---
struct CGI {
  string method, query, body, cookie;
  map<string,string> params;
  CGI() {
    const char* m = getenv("REQUEST_METHOD"); method = m?m:"GET";
    const char* q = getenv("QUERY_STRING"); query = q?q:"";
    const char* ck = getenv("HTTP_COOKIE"); cookie = ck?ck:"";
    if (method=="POST") {
      int len = atoi(getenv("CONTENT_LENGTH")?getenv("CONTENT_LENGTH"):"0");
      body.resize(len);
      if (len) fread(&body[0],1,len,stdin);
    }
  }
  static string url_decode(const string& s){
    string o; for(size_t i=0;i<s.size();++i){
      if(s[i]=='+' ) o.push_back(' ');
      else if(s[i]=='%' && i+2<s.size()){
        int v; sscanf(s.substr(i+1,2).c_str(),"%x",&v);
        o.push_back(static_cast<char>(v)); i+=2;
      } else o.push_back(s[i]);
    } return o;
  }
  static map<string,string> parse_kv(const string& s){
    map<string,string> m; stringstream ss(s); string kv;
    while(getline(ss, kv, '&')){
      auto p = kv.find('=');
      string k = url_decode(p==string::npos?kv:kv.substr(0,p));
      string v = url_decode(p==string::npos?"":kv.substr(p+1));
      m[k]=v;
    } return m;
  }
  void parse(){
    params = parse_kv(query);
    if (method=="POST"){ auto pp=parse_kv(body); params.insert(pp.begin(),pp.end()); }
  }
};

// --- DB ---
struct DB {
  MYSQL* conn;
  DB() {
    conn = mysql_init(NULL);
    const char *h=getenv("DB_HOST"), *u=getenv("DB_USER"), *p=getenv("DB_PASS"), *d=getenv("DB_NAME");
    if(!mysql_real_connect(conn,h?h:"localhost",u?u:"root",p?p:"",d?d:"auction_db",0,NULL,0)){
      throw runtime_error(mysql_error(conn));
    }
    mysql_set_character_set(conn,"utf8mb4");
  }
  ~DB(){ if(conn) mysql_close(conn); }
  void exec(const string& q){ if(mysql_query(conn, q.c_str())) throw runtime_error(mysql_error(conn)); }
  string esc(const string& s){
    string out; out.resize(s.size()*2+1);
    unsigned long l=mysql_real_escape_string(conn,&out[0],s.c_str(),s.size());
    out.resize(l); return out;
  }
};

// --- Helpers ---
static string sha256(const string& s){
  unsigned char h[SHA256_DIGEST_LENGTH];
  SHA256((const unsigned char*)s.data(), s.size(), h);
  return string((char*)h, SHA256_DIGEST_LENGTH);
}
static string random_bytes(size_t n){
  ifstream ur("/dev/urandom", ios::binary);
  string s(n,'\0'); ur.read(&s[0], n); return s;
}
static string hex(const string& bin){
  static const char* d="0123456789abcdef";
  string o; o.reserve(bin.size()*2);
  for(unsigned char c: bin){ o.push_back(d[c>>4]); o.push_back(d[c&15]); }
  return o;
}
static string get_cookie(const string& hdr,const string& key){
  string k=key+"="; stringstream ss(hdr); string part;
  while(getline(ss,part,';')){ while(!part.empty()&&part.front()==' ') part.erase(part.begin());
    if(part.rfind(k,0)==0) return part.substr(k.size()); }
  return "";
}
static string now_sql_utc(){ return "UTC_TIMESTAMP()"; }

static optional<int> require_session(DB& db,const CGI& cgi,bool refresh=true){
  string sid=get_cookie(cgi.cookie,"ASESSION");
  if(sid.empty()) return nullopt;
  string q="SELECT user_id, UNIX_TIMESTAMP(UTC_TIMESTAMP())-UNIX_TIMESTAMP(last_activity) "
           "FROM sessions WHERE id='"+db.esc(sid)+"'";
  db.exec(q); MYSQL_RES* r=mysql_store_result(db.conn); MYSQL_ROW row=mysql_fetch_row(r);
  if(!row){ mysql_free_result(r); return nullopt; }
  int uid=atoi(row[0]); long idle=atol(row[1]); mysql_free_result(r);
  if(idle>300){ db.exec("DELETE FROM sessions WHERE id='"+db.esc(sid)+"'"); return nullopt; }
  if(refresh) db.exec("UPDATE sessions SET last_activity="+now_sql_utc()+" WHERE id='"+db.esc(sid)+"'");
  return uid;
}

static void hdr_ok(const string& extra=""){ cout<<"Content-Type: text/html; charset=utf-8\r\n"<<extra<<"\r\n"; }
static void page(const string& title,const string& body){
  cout<<"<!doctype html><html><head><meta charset='utf-8'><title>"<<title
      <<"</title><link rel='stylesheet' href='/style.css'></head><body><div class='wrap'>"
      <<body<<"</div></body></html>";
}
